...
sentinel_cfg = ArionConfig.tokenize(sentinel_config_file)
sentinel_version = "1.3.0"
min_ariond_proto_version_with_sentinel_ping = 70211


def get_arion_conf():
    if sys.platform == 'win32':
        arion_conf = os.path.join(os.getenv('APPDATA'), "ArionCore/arion.conf")
    else:
        home = os.environ.get('HOME')

        arion_conf = os.path.join(home, ".arion/arion.conf")
        if sys.platform == 'darwin':
            arion_conf = os.path.join(home, "Library/Application Support/ArionCore/arion.conf")

    arion_conf = sentinel_cfg.get('arion_conf', arion_conf)
