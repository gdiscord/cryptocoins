...
def is_valid_arion_address(address, network='mainnet'):
    ...
    arion_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Arion address
        return False

    if (address_version != arion_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('arion.conf')
