...
class ArionDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some ariond methods
    ...
    @classmethod
    def from_arion_conf(self, arion_dot_conf):
        from arion_config import ArionConfig
        config_text = ArionConfig.slurp_config_file(arion_dot_conf)
        creds = ArionConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = arionlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = arionlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = arionlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_ariond_proto_version_with_sentinel_ping)
