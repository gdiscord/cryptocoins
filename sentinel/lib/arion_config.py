...
class ArionConfig():
    ...
    def slurp_config_file(self, filename):
        # read arion.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is arion.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from arion.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Arion defaults...
        default_port = 6269 if (network == 'mainnet') else 16269

        # use default port for network if not specified in arion.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from arion.conf
