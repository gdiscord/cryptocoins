...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            if not arionlib.is_valid_arion_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Arion address for network [%s], returning False" % (self.payment_address, config.network))
                return False
