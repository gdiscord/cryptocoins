...
def has_arion_conf():
    ...
    valid_arion_conf = False

    # ensure arion_conf exists & readable
    #
    # if not, print a message stating that Arion Core must be installed and
    # configured, including JSONRPC access in arion.conf
    try:
        f = io.open(config.arion_conf)
        valid_arion_conf = True
    except IOError as e:
        print(e)

    return valid_arion_conf
...
def main():
    ...
    if not has_arion_conf():
        print("ArionCore must be installed and configured, including JSONRPC access in arion.conf")
        sys.exit(1)
