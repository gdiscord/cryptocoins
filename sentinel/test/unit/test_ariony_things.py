...
def test_valid_arion_address():
    from arionlib import is_valid_arion_address

    main = valid_arion_address()
    test = valid_arion_address('testnet')

    assert is_valid_arion_address(main) is True
    assert is_valid_arion_address(main, 'mainnet') is True
    assert is_valid_arion_address(main, 'testnet') is False

    assert is_valid_arion_address(test) is False
    assert is_valid_arion_address(test, 'mainnet') is False
    assert is_valid_arion_address(test, 'testnet') is True


def test_invalid_arion_address():
    from arionlib import is_valid_arion_address

    main = invalid_arion_address()
    test = invalid_arion_address('testnet')

    assert is_valid_arion_address(main) is False
    assert is_valid_arion_address(main, 'mainnet') is False
    assert is_valid_arion_address(main, 'testnet') is False

    assert is_valid_arion_address(test) is False
    assert is_valid_arion_address(test, 'mainnet') is False
    assert is_valid_arion_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from arionlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % arionlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(arionlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(arionlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(arionlib.blocks_to_seconds(16616)) == 2612035
